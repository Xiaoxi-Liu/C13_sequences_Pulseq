function [k,dcf,t,ind,out,grad]=design_spiral_pulseq(fov,npix,arms,ksamp,...
    fname,gmax,smax,acq_round2n,do_rot_file,balanced)
...
%       gmax  Max gradient amp                        (33)   [Hz/m]
%       smax  Max slew rate                           (120)  [Hz/m/s]
...
%
% created by 7/2019 Rolf Schulte
% modified by 1/2024 Xiaoxi Liu
...
fufa_gmax = 0.99;
fufa_smax = 0.99;
g_offset = 1;               % #grad pts offset for start of trajectory
...
if isempty(gmax),               gmax = 3.5e5; end   % [Hz/m]
if ~exist('smax','var'),        smax = []; end
if isempty(smax),               smax = 120e7; end  % [Hz/m/s]
...
gmax = gmax/10708000;         % [mT/m] -> [T/m]
smax = smax/10708000;         % [Hz/m/s] -> [T/m/s]
...
gmax_nyquist = 2*pi/(267.522128d6*ksamp*fov_mns);
...
if balanced
    grewx = gradient_lobe(-real(kspir(1,end)),gsamp,...
        fufa_gmax*gmax/sqrt(2),fufa_smax*smax/sqrt(2),...
        real(gspir(1,end)),false);
    grewy = gradient_lobe(-imag(kspir(1,end)),gsamp,...
        fufa_gmax*gmax/sqrt(2),fufa_smax*smax/sqrt(2),...
        imag(gspir(1,end)),false);
...
%% k-space trajectory
if dki>1, k = kspir(1:dki:length(kspir))/2/pi*res_mns*267.522128d6; 
else,     k = kspir/2/pi*res_mns*267.522128d6;
end
if abs(dki-0.5)<1d-10
